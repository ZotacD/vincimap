...
# Arguments
def load_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="VinciMap video-to-3D reconstruction pipeline.",
    ...
    if args.video_frame_rate <= 0:
        parser.error("--video_frame_rate must be greater than 0.")

    if args.trainer_data_factor <= 0:
        parser.error("--trainer-data_factor must be greater than 0.")
...
def extract_video_images(
    ...
    if trainer_data_factor > 1:
        scaled_images_path = Path(
            f"{images_path}_{trainer_data_factor}"
        )

        ffmpeg_utils.images_to_scaled_images(
            images_path,
            scaled_images_path,
        )
...
def merge_sparse_submodels(
    ...
    if not submodels:
        raise FileNotFoundError(
            f"No sparse submodels found in: {sparse_path}"
        )
    ...
    for submodel_path in submodels[1:]:
        ...
        print(
            f"Merging sparse {submodel_path.name} "
            f"with sparse {merged_model_path.name}"
        )
...
def run_gaussian_training(
    ...
    trainer_args.extend(
        ["--data_factor", str(trainer_data_factor)]
    )
...
def run_pipeline_from_step(
    ...
    for current_step in steps_to_run:
        ...
        print(
            f"[VinciMap] Running step: {current_step}"
        )
        ...
        print(
            f"[VinciMap] Completed step: {current_step}"
        )
...
def main(
    ...
    if (
        args.workspace_path is not None
        and args.video_path is not None
    ):
        raise ValueError(
            "Arguments --workspace_path and --video_path "
    ...
    if args.workspace_path is not None:
        ...
        if args.video_path is None:
            raise ValueError(
                "Argument --video_path is required "
                "when --workspace_path is not provided."
            )
    ...
    if video_path is not None:
        ...
        video_frame_rate = min(
            math.ceil(
                ffmpeg_utils.frame_rate_from_video(
                    video_path
                )
            ),
    ...
    if (
        video_path is None
        and step == "extract_video_images"
    ):
        raise ValueError(
            "Argument --video_path is required "
            "to run step extract_video_images."
        )
...
if __name__ == "__main__":
    ...
    if not colmap_utils.colmap_installed(
        args.colmap_path
    ):

        raise EnvironmentError(
            "COLMAP is not installed or not accessible from the system. "
            "Please install COLMAP and make sure it is available in the PATH "
            "or pass it with --colmap_path."
        )
    ...
    if not torch.cuda.is_available():
        ...
        raise EnvironmentError(
            ...
            "Please check that CUDA is installed correctly, "
